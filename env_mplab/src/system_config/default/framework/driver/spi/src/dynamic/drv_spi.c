    ...
    if (pDrvInstance->numClients != 0)
    {
        if ((intent & DRV_IO_INTENT_EXCLUSIVE) == DRV_IO_INTENT_EXCLUSIVE) {
            ...
            if (OSAL_SEM_Post(&sSPIManager.managerSemaphore) != OSAL_RESULT_TRUE) {
                SYS_CONSOLE_MESSAGE("\r\nSPI Driver: OSAL Fail IO_Exclusive to already client")
                return DRV_HANDLE_INVALID;
                /* Report Error*/
            }
            SYS_CONSOLE_MESSAGE("\r\nSPI Driver: IO_Exclusive to already client")
            return DRV_HANDLE_INVALID;
        }
        if (pDrvInstance->isExclusive) {
            SYS_CONSOLE_MESSAGE("\r\nSPI Driver: Trying to open a second client to a driver that is in exclusive mode.");
            if (OSAL_SEM_Post(&sSPIManager.managerSemaphore) != OSAL_RESULT_TRUE) {
                SYS_CONSOLE_MESSAGE("\r\nSPI Driver: OSAL Fail Second Client to IO_Exclusive")
                return DRV_HANDLE_INVALID;
                /* Report Error*/
            }
            SYS_CONSOLE_MESSAGE("\r\nSPI Driver: Second Client to IO_Exclusive")
            return DRV_HANDLE_INVALID;
        }
    } else {
        SYS_CONSOLE_MESSAGE("\r\nSPI Driver: Setup Hardware")
        setupHardware = true;
    }
