...
void RGBLED_Init()
{
    ...
    // RGBLED_Timer5Setup(); // for PDM usage
    lat_LED8_R = 0;
    lat_LED8_G = 0;
    lat_LED8_B = 0;
...
**          
*/
void RGBLED_SetValue(uint8_t bValR, uint8_t bValG, uint8_t bValB)
{
//    /* for PDM usage */
//    bColR = bValR;
//    bColG = bValG;
//    bColB = bValB;
    /* for PWM usage */
    OC3RS = bValR * (PR2 + 1) / 255;
    OC5RS = bValG * (PR2 + 1) / 255;   
    OC4RS = bValB * (PR2 + 1) / 255;
}
